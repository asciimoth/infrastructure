...
echo $CRED >> $CREDFILE
echo $'\n' >> $CREDFILE
expect -c 'spawn fido2luks -i add-key $::env(PARTITION) $::env(CRED) --pin; expect "Authenticator PIN:"; send "$::env(DEVICEPIN)\r"; expect "Current password:"; send "$::env(PASSWORD)\r"; expect "New password:"; send "$::env(NEWPASS)\r"; interact'
